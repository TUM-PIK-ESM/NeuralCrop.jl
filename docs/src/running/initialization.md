...
A simulation usually loads data, builds an `InitialData` batch with `InitialDataLoader`, initializes state with `init_states!`, spins up climate buffers with `spin_up_climbuf!`, and then calls a daily simulation driver.
