function soil_carbon!(nn_model,
    ...
    soil.litc, soil.decom_litc = hybrid_litc(nn_model.lit, soil.litc, ps.litd, st.litd, input, soil.respose_litc)
    ...
    soil.fastc, soil.decom_fastc = hybrid_soilc(nn_model.soil, soil.fastc, ps.soild, st.soild, input, soil.respose_fastc, soil.c_shift_fast, sum(soil.decom_litc, dims = 1))
    soil.slowc, soil.decom_slowc = hybrid_soilc(nn_model.soil, soil.slowc, ps.soild, st.soild, input, soil.respose_slowc, soil.c_shift_slow, sum(soil.decom_litc, dims = 1))
