...
# Hybrid modelling
function C4_crop!(day_start, day_end,
                  model, ps, st,
                  pftparameters,
                  climate, climbuf, crop, crop_cal, photos, pet, soil, managed_land,
                  dailyWeather, output;
                  node = true, maize = true, irrigation = false
)
    ...
    for day = day_start : day_end
        ...
        # soil water cycle
        soil_water!(model.swc, ps_frozen.swc, st.swc, soil, crop, dailyWeather.prec, dailyWeather.swr_n, dailyWeather.lwr_n; irrigation = irrigation)
